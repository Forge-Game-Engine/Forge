...
export function setupAnimationsDemo(
  ...
  //left column
  const shipAnimationSet = createShipAnimations();
  buildShipEntities(world, shipSprite, shipAnimationSet);
  ...
  // //right column
  // createAdventurerControllableAnimations();
...
function createShipAnimations() {
  ...
  const shipAnimationSet = new AnimationSet(ENTITY_TYPES.ship, [
    shipSpinRandomAnimation,
    shipSpinAnimation,
  ]);

  return shipAnimationSet;
}
...
function buildShipEntities(
  ...
  world.buildAndAddEntity('ship-animation-spin', [
    ...
    new SpriteAnimationComponent(animationSet),
  ]);
