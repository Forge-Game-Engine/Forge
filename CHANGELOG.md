...
- **input:** Add a canvas-space pointer: `PointerEcsComponent`/`addPointerComponent` plus `createPointerEcsSystem`, which copies a `PointerInputSource`'s position, movement delta, scroll, and button down/held/up state into every pointer component once per tick. `MouseInputSource` implements `PointerInputSource` and exposes this state directly (`position`, `delta`, `scroll`, `buttonsDown`, `buttonsHeld`, `buttonsUp`) for code that wants it without going through the ECS
