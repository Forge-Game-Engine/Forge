...
- **text:** Add outline and soft-shadow/glow effects (Phase 4 of MSDF text rendering): `TextEcsComponent.outlineColor`/`outlineWidth` and `shadowColor`/`shadowOffset`/`shadowSoftness`, drawn by the MSDF fragment shader with no additional draw call, in screen-pixel-range units so an effect's size stays constant on screen regardless of camera zoom or entity scale. Each glyph's own effect size is automatically clamped both to the font atlas's own encoded `distanceRange` budget and to the gap to its tightest same-word neighbor's own ink, computed at shape time, so a requested `outlineWidth`/`shadowSoftness` can safely reach right up to an adjacent glyph without ever painting over it - see the new Text Effects doc for the safe range, its two independent limits, and how it scales with the text's on-screen size
