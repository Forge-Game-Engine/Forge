...
- **text:** Add outline and soft-shadow effects (Phase 4 of MSDF text rendering): `TextEcsComponent` gains `outlineColor`/`outlineWidth` and `shadowColor`/`shadowOffset`/`shadowSoftness`, drawn by the MSDF fragment shader as a near-free extra of the existing glyph fill, with no additional draw call - text entities sharing a `FontAtlas` still batch together even when their outline/shadow settings differ
