 ...
 * exactly 1 atlas pixel per world unit, see `text-effects.ts`) of the
 * generated square glyph's tile.
 */
export const SYNTHETIC_GLYPH_TILE_SIZE = 128;

/**
 * The half-width/height, in atlas pixels, of the glyph's own filled ink
 * square, centered within the tile. The gap between the ink and the tile's
 * edge (`SYNTHETIC_GLYPH_TILE_SIZE / 2 - SYNTHETIC_GLYPH_INK_HALF_SIZE`) is
 * deliberately much larger than `SYNTHETIC_GLYPH_DISTANCE_RANGE / 2`, so the
 * distance field genuinely saturates well before reaching the tile edge -
 * the same shape of headroom a real MSDF atlas's glyph padding leaves (see
 * `msdf.frag.glsl`'s `maxSafeEffectDistance`).
 */
