...
describe('createRevoluteJointEcsSystem', () => {
  ...
  it('keeps a dynamic body pinned to a static pivot under repeated gravity-like kicks', () => {
    ...
    const ballPosition = addPositionComponent(world, ball, {
      world: new Vector2(0, -5),
      local: new Vector2(0, -5),
    });
    ...
    addRevoluteJointComponent(world, jointEntity, {
      entityA: pivot,
      entityB: ball,
      localAnchorB: new Vector2(0, 5),
    });

    for (let i = 0; i < 180; i++) {
      ballRigidBody.velocity = ballRigidBody.velocity.add(
        new Vector2(0, -9.8 * (1 / 60)),
      );
      tick();
    }

    const anchorB = ballPosition.world.add(
      new Vector2(0, 5).rotate(ballRotation.world),
    );
    const separation = anchorB.magnitude();
  ...
  it('keeps accumulated point impulse bounded (does not diverge) across many ticks', () => {
    ...
    addPositionComponent(world, ball, {
      world: new Vector2(3, 0),
      local: new Vector2(3, 0),
    });
    ...
    expect(joint.accumulatedPointImpulse.magnitude()).toBeLessThan(1000);
  });
});
