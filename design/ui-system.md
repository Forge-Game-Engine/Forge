...
a _different_ world Y than the panel's own anchor point, so the two routinely
...
reason — `InputsEcsComponent` holds a _reference_ to an `InputManager`
...
| #   | Item                                                                                                                                                                                                                                                                                    | Size |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| 3.1 | **Landed.** `UiToggleEcsComponent`/`UiToggleGroupEcsComponent`/`createUiToggleEcsSystem`/`createToggle` (checkbox / radio via toggle groups)                                                                                                                                            | S    |
| 3.2 | **Landed.** `UiSliderEcsComponent`/`createUiSliderEcsSystem`/`createSlider` (drag handle, fill, min/max, whole-number mode)                                                                                                                                                             | M    |
...
| 3.6 | **Landed.** `UiDropdownEcsComponent`/`createDropdown` - no generic system (see the component's own doc comment for why); closing on an outside click is a known limitation, not implemented                                                                                             | M    |
| 3.7 | **Landed** as a linear fill: `UiProgressBarEcsComponent`/`createUiProgressBarEcsSystem`/`createProgressBar`. Radial fill deferred - it needs a shader-level fill-amount uniform, not just a rect resize like the linear case, real render-pipeline work disproportionate to an "S" item | S    |
