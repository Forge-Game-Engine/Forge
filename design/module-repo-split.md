...
| `@forge-game-engine/forge` (root package)                       | Modified | Becomes a thin umbrella/meta-package that depends on and re-exports every extracted package, so existing `@forge-game-engine/forge/<module>` imports keep resolving. Hosts `/demo`, `/documentation-site`, and `/e2e` going forward.                      |
...
| No module is removed by this design.                            |          |                                                                                                                                                                                                                                                           |
...
- How `/demo`, `/documentation-site`, and `/e2e` (which currently consume the engine via a
  single `file:..` dependency) adapt to consuming multiple packages.
- Backward compatibility strategy for existing consumers of `@forge-game-engine/forge`.
...
| Umbrella package                                | The existing `@forge-game-engine/forge` root package becomes a workspace member that depends on every other workspace package and re-exports their export paths, so `@forge-game-engine/forge/physics` keeps resolving unchanged. | M    |
| Validate `/demo`, `/documentation-site`, `/e2e` | Confirm all three still build and run against the umbrella package with no code changes required, since its public surface is unchanged.                                                                                          | M    |

Definition of done: `npm run check-types && npm test && npm run lint && npm run cspell &&
npm run check-exports` all pass against the new workspace layout, `/demo` and
`/documentation-site` build and run unmodified, and the published `dist` output is
byte-for-byte equivalent in its public API surface to what `tsc --project
tsconfig.build.json` produces today.
...
| Umbrella package re-export update                      | `@forge-game-engine/forge` now depends on the three published packages instead of building them itself.                                                                                                                            | S    |
...
same way an external consumer would — over the registry, not a workspace link. The umbrella
...
| Umbrella package re-export update      | `@forge-game-engine/forge` depends on the published `core` package.                                                                                                                                                                                                   | S    |
...
in-monorepo module over the registry; the full verification checklist passes; `/demo` and
`/documentation-site` behave identically to before the phase.
...
| Umbrella package re-export update                                                | `@forge-game-engine/forge` depends on all newly published packages.                                                                          | S    |
| Retire the workspace layout                                                      | `/packages` is removed from the main repo; only the umbrella package (and `/demo`, `/documentation-site`, `/e2e`) remain.                    | M    |

Definition of done: every module identified in this design has an independent repo,
package, and release history; the main `Forge` repo contains only the umbrella package,
demo app, documentation site, and e2e suite.

### Phase 5 — Consumer migration and cleanup

Goal: give consumers a path off the umbrella package, onto the individual packages they
actually use, and decide the umbrella package's long-term fate.

| Task                               | Description                                                                                                                                                          | Size |
| ---------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---- |
| Update `/demo`                     | Depend on the individual packages it actually uses instead of the umbrella package, as a worked example for consumers.                                               | M    |
| Update `/documentation-site` demos | Same, per demo page — see AGENTS.md's "Documentation Site Demos" section for why each one needs its own verification pass.                                           | L    |
...
| Decide the umbrella package's fate | Per [Open Questions](#open-questions): keep it as a permanent convenience re-export, deprecate it on a timeline, or drop it immediately since the engine is pre-1.0. | S    |

Definition of done: the migration guide is published; `/demo` and `/documentation-site`
depend on individual packages; the umbrella package's long-term status is decided and
documented.
...
2. **Umbrella package: permanent, deprecated-on-a-timeline, or dropped?** AGENTS.md notes the
   engine is pre-1.0 and doesn't require upgrade guides for breaking changes at this stage,
   which argues for a clean cut to individual packages. But `/demo` and
   `/documentation-site` (and any early external consumers) currently depend on the single
   `@forge-game-engine/forge` package, so _something_ needs to keep working during the
   multi-release window this plan spans. Needs a decision before Phase 5.
3. **Cross-package integration testing.** Today, `npm test` in one repo sees the whole
   ...
   automatically. Candidates: a scheduled job in the umbrella repo that installs the latest
...
4. **Are GitHub Packages publish credentials shared across repos, or per-repo?** The current
...
5. **Does `/e2e` move, split, or stay?** `/e2e` currently imports straight from `/src`
   (deliberately, per AGENTS.md, to stay unaffected by demo/docs changes) and exercises
   rendering, input, and the game loop together — i.e. exactly the modules landing in
   `core`. It likely stays with whichever repo hosts `core`, or the umbrella repo once it
   depends on published `core`; needs a decision by Phase 3.
6. **Should `core` be untangled further, and when?** This design deliberately stops at
...
| --------------------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
| `Forge` (existing, becomes the umbrella repo) | `@forge-game-engine/forge`                                                                                          |
...
- `/e2e`'s Playwright suite, which today imports straight from `/src` to stay independent of
  demo/docs changes, needs to import from the published `core` (and any other) packages
  once the split completes, since there's no longer a `/src` to import from directly in the
  umbrella repo. This is a deliberate trade: e2e becomes a true integration test against
