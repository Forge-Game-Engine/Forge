...
`registerUiSystems`) once it actually shapes the label's text, may not exist
yet on the very first tick a brand-new label is created - that tick measures
as zero-width instead of throwing, the same one-frame lag every
freshly-created layout group child already has. An explicit
