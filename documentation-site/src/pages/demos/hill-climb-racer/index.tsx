...
export default function HillClimbRacer(): JSX.Element {
  return (
    <Demo
      ...
      blurb="This demo composes several of the physics engine's existing primitives into a drivable car: the chassis and both wheels are independent RigidBody instances (no rigid frame connecting them), and each wheel is driven by an AngularVelocityMotorEcsComponent (see the Torque and Motors demo) whose target speed tracks the throttle input. Each wheel hangs below the chassis on two LinearSpring/LinearDamper pairs (see the Linear Spring and Damper demo - the same components used there for vehicle suspension), anchored to two points spread fore-and-aft on the chassis rather than one: a single anchor only constrains the wheel's distance from that point, leaving it free to swing around it like a pendulum, while two anchors triangulate its position the way a real wishbone's two pivots do. Because the wheels are only ever connected to the chassis through soft springs, accelerating and braking visibly pitches the chassis backward and forward, the 'leaning' feel the genre is named for. That two-spring mount still has no equivalent of a real suspension's rigid control-arm geometry, so small disturbances can nudge the chassis off level over time; a light demo-only ChassisStabilizerEcsComponent nudges it back once nothing else is actively tipping it, without fighting a deliberate acceleration or brake lean. The terrain is a chain of static ground segments following a procedurally generated height profile, and a demo-only camera-follow system (the engine's built-in camera only supports input-driven pan/zoom) keeps the car in view as it drives across a course much wider than the canvas. The hills get steep further along the course - if the car flips or gets stuck, press R to restart."
