...
/**
 * Creates static rigid bodies for the top, left and right walls, bounding
 * the area the ball bounces around in. The bottom is left open so a ball
 * that gets past the paddle can fall out of play.
 * @param world - The ECS world to add the wall entities to.
 * @param renderContext - The render context used to load the wall sprite.
 * @param renderLayer - The render layer the walls should be drawn on.
 * @returns The play area the walls bound.
 */
export async function createBoundaries(
  ...
  return {
    ...
    maxX: halfWidth - wallThickness,
    topY: halfHeight - wallThickness,
    bottomY: -halfHeight,
  };
}
