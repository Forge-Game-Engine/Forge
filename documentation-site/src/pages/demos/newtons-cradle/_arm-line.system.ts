 ...
 * `RigidBody.angle` tracks its spin, not the pendulum's swing angle, so the
 * arm's direction has to be derived from the two positions instead). Resized
 * directly via `SpriteEcsComponent.width`/`height` (rather than a
 * non-uniform `ScaleEcsComponent`) so the sprite's nine-sliced end caps stay
 * a fixed size instead of smearing as the arm swings. Must run before
 * `createRenderEcsSystem` so the render pass sees this tick's updated arm,
 * and after `createPhysicsSyncEcsSystem` so `body.position` reflects this
 * tick's physics step.
 */
...
> => ({
  ...
  run: (result) => {
    ...
    // local "up" (0, 1) axis - see how `_create-cradle.ts`/
    // `_create-wrecking-ball.ts` place a ball from a pivot and angle
    // (`pivotPosition.subtract(localAnchorB.rotate(angle))`) for the
    // matching forward calculation this undoes.
